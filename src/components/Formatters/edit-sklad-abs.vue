<template>
    <div :style="{ cursor: 'pointer', backgroundColor: !!checked ? '#b8ffb8' : '' }">
      <el-dialog
        title="Кол-во ЗИП на складе ABS"
        :visible.sync="show"
        :close-on-click-modal="false"
        destroy-on-close
        width="450px"
        >
        <div v-if="info">Редактор: {{  info.user || 'нет'  }}</div>
...
import { update, getInfo, sendRequest } from '@/api/market/sklad-abs.js'
...
export default {
  ...
  mounted() {
    ...
    this.checked = this.row['marketABSChecked']
    ...
    save() {
      ...
      update(data)
        .then(res => {
          ...
          this.row[this.column] = this.qty + " (" + this.shelf + ' - ' + this.place + ")";
          // eslint-disable-next-line vue/no-mutating-props
          this.row["marketABSComment"] = this.comment;
          // eslint-disable-next-line vue/no-mutating-props
          this.row["marketABSAddress"] = this.shelf + '-' + this.place;
        })
        .catch(err => {
      ...
      getInfo({ market_id: this.row.marketid })
        .then(res => {
          ...
          this.info = res.map(item => {
            ...
            return { user: getUserName(item.user), comment: item.comment, date: item.date ? new Date(item.date).toLocaleDateString("ru") : 'нет' }
          })[0];
