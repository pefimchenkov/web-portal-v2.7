<template>
  ...
  >
    ...
    <v-toolbar color="#eaeff5" class="text-lg-right elevation-2 mb-1" dense style="width: 100%">
      <v-toolbar-title>Производители</v-toolbar-title>
...
import { addOne, updateOne, deleteOne } from '@/api/models/vendors.js'
import { getAll } from '@/api/models/vendors'
  ...
  methods: {
    ...
    async add(form) {
      addOne({ ...form, displayName: getUserName(this.user.email), })
        .then(async res => {
          if (!res.insertId) {
            return this.$notify({ type: 'error', message: 'Произошла ошибка в процессе добавления Производителя.' })
          }
