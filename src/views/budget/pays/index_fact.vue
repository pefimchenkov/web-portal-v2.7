<template>
  ...
  <v-toolbar color="#eaeff5" class="text-lg-right elevation-2 mb-1" dense style="width: 100%">
    <v-toolbar-title>Фактические расходы</v-toolbar-title>
      ...
      <el-dropdown-menu slot="dropdown">
        <el-dropdown-item>
          <TableColumns
            ...
            name="budgetOutgoingPaymentsColumn"
          />
        </el-dropdown-item>
    ...
    >
      ...
      <template v-if="column.formatter" #default="{ row }">
        <component
          ...
          :row="row"
          :column="column.value"
          :options="{ cfAnalitic: CfAnalitics }"
...
<script>
  ...
  import { templateHeaders, reference, exportFileName, dialogItems, dialogOptions } from './data_fact.js'
  ...
  export default {
    ...
    async created() {
      ...
      if (!this.OutgoingPayments?.length)
        await this.$store.dispatch('atlas_1c/getAtlasOutgoingPayments', dates)
      ...
      //console.log(this.OutgoingPayments[0])

      console.log(this.OutgoingPayments)

      const commonData = this.OutgoingPayments || [];
  
      /* const commonData = this.OutgoingPayments.map((item) => {
    ...
    methods: {
      ...
      getSumWithRate(sum, date, curr, rate) {
       ...
       formatterData(row, column, cellValue) {
        if (column.property === 'paymentDate' || column.property === 'subjectDate' ) {
          return cellValue ? new Date(cellValue).toLocaleDateString('ru') : null;
        }
  
        if (column.property === 'paymentSum' || column.property === 'sum') {
